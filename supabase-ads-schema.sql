...
CREATE TABLE IF NOT EXISTS public.ads (
    ...
    priority INTEGER DEFAULT 0, -- Higher priority ads show first
...
-- Function to redeem ad credits
CREATE OR REPLACE FUNCTION redeem_ad_credits(
    p_user_id UUID,
    p_redemption_type VARCHAR(50),
...
BEGIN
    ...
    -- Calculate redemption amount based on type
    CASE p_redemption_type
        WHEN 'tag_discount' THEN v_amount := p_credit_amount; -- 1:1 credit to discount
        WHEN 'premium_trial' THEN v_amount := 5.00; -- $5 for premium trial
        WHEN 'cash_out' THEN v_amount := p_credit_amount * 0.9; -- 90% of credit value
        ELSE
            RETURN jsonb_build_object('success', false, 'error', 'Invalid redemption type');
    END CASE;
    
    -- Create redemption record
    INSERT INTO public.ad_redemptions (user_id, redemption_type, amount, credit_amount, description)
    VALUES (p_user_id, p_redemption_type, v_amount, p_credit_amount, p_description)
    RETURNING id INTO v_redemption_id;
    
    -- Update user's credit balance
    UPDATE public.ad_credits 
    SET credit_balance = credit_balance - p_credit_amount,
        total_redeemed = total_redeemed + p_credit_amount,
        last_redeemed = NOW(),
        updated_at = NOW()
    WHERE user_id = p_user_id;
    ...
    RETURN jsonb_build_object(
        ...
        'amount', v_amount,
        'remaining_balance', v_current_balance - p_credit_amount
    );
EXCEPTION
    WHEN OTHERS THEN
        RETURN jsonb_build_object(
...
-- Reset daily view counts (this would typically be run by a cron job)
CREATE OR REPLACE FUNCTION reset_daily_ad_counts()
RETURNS void AS $$
BEGIN
    UPDATE public.ads SET current_daily_views = 0;
END;
$$ LANGUAGE plpgsql; 